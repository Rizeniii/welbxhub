if (game:GetService("CoreGui")):FindFirstChild("Welb X Hub") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	(game:GetService("CoreGui")).Welb X Hub:Destroy();
...
ImageButton.MouseButton1Click:connect(function()
	(game.CoreGui:FindFirstChild("Welb X Hub")).Enabled = not (game.CoreGui:FindFirstChild("Welb X Hub")).Enabled;
end);
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("Welb X Hub") then
			makefolder("Welb X Hub");
		end;
		if not isfolder("Welb X Hub/Library/") then
			makefolder("Welb X Hub/Library/");
		end;
		if not isfile(("Welb X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("Welb X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("Welb X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("Welb X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("Welb X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
function Update:Window(Config)
	...
	local Welb X Hub = Instance.new("ScreenGui");
	Welb X Hub.Name = "Welb X Hub";
	Welb X Hub.Parent = game.CoreGui;
	Welb X Hub.DisplayOrder = 999;
	local OutlineMain = Instance.new("Frame");
	OutlineMain.Name = "OutlineMain";
	OutlineMain.Parent = Welb X Hub;
	...
	CreateButton("Reset Config", function()
		if isfolder("Welb X Hub") then
			delfolder("Welb X Hub");
		end;
